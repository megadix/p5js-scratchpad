...
function initBodies() {
  const newBody = (name, mass, posx, vely, col) => {
    if (name !== 'Sun') {
      maxMass = max(maxMass, mass);
    }
    return new Body(
      name, mass,
      createVector(posx, 0.0),
      createVector(0.0, vely),
...
function update() {
  ...
  Object.values(bodies).forEach(body1 => {
    body1.force = createVector(0.0, 0.0);
...
function mouseWheel(event) {
  ...
  zoom = constrain(zoom, maxZoomIn, maxZoomOut)
}
...
function draw() {
  ...
  Object.values(bodies).forEach(body => {
    ...
    const radius = (body.name === 'Sun') ? 5 : map(body.mass, minMass, maxMass, 1, 4);

    circle(
      map(body.position.x, -zoom, zoom, 0, width),
      map(body.position.y, -zoom, zoom, 0, height),
      radius
    );

    circle(10, texty, radius);
    textSize(9);
    textAlign(LEFT, CENTER);
    text(body.name, 20, texty);
